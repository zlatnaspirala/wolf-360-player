function VIDEO_TEXTURE(monitor_) {
  ...
  ROOT.AUTO_UPDATE = function() {
    ...
    if (ROOT.video.readyState === ROOT.video.HAVE_ENOUGH_DATA) {
      ...
      if (ROOT.videoTexture) ROOT.videoTexture.needsUpdate = true;
    }
  };

  console.log("Video 3d canvas texture created. Setup TITLE innerHTML.");
  var TITLE = document.getElementById("wolf-player-title");
  TITLE.innerHTML = TITLE.innerHTML.replace("Loading", "Playing");

  DIV_CONTENT_STREAMS.style.display = "none";
  ROOT.video.play();
