...
if ("webkitSpeechRecognition" in window) {
  SYS.VOICE.LISTEN = function() {
    ...
    ROOT_LISTENER.recognition.onstart = function() {
      console.log("START");
    };
    ...
    ROOT_LISTENER.recognition.onend = function() {
      console.log("ON END");
    };
          ...
          if (event.results[i][0].transcript.indexOf(" ") != -1) {
            ...
            for (var x = 0; x < ROOT_LISTENER.embbed_action.length; x++) {
              if (typeof event.results[i][0].transcript.indexOf(ROOT_LISTENER.embbed_action[x]) != -1) {
                eval(
                  ' if (typeof  ROOT_LISTENER.embbed_action[x]  != "undefined" )  {   window[ROOT_LISTENER.embbed_action[x]]() ;   } '
                );
              }
            }
            ...
            eval(
              " if (typeof  ROOT_LISTENER.words." +
                event.results[i][0].transcript +
                ' != "undefined" )  {   ROOT_LISTENER.words.' +
                event.results[i][0].transcript +
                ".action();   } "
            );
          }
